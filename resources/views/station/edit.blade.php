...
@section('content')
    <x-card>
        <form method="post" action="{{ route('station.update', $station->id) }}" class="" id="submit-form">
            @csrf
            @method('put')
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" class="form-control">{{ old('description', $station->description) }}</textarea>
            </div>

            <div class="form-group">
                <x-input-label for="location" value="Location" />
                <x-text-input id="location" name="location" type="text" class="tw-mt-1 tw-block tw-w-full location" :value="old('location', $station->latitude.', '.$station->longitude)" />
